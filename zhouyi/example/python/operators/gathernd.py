...
def test_gathernd_example_int16():
    ...
    expected_output = np.array([0, 3], dtype=np.int16)
    assert np.array_equal(output, expected_output)
    expect(
        node,
        inputs=[data, indices],
        outputs=[output],
        name="test_gathernd_example_int16",
        initializer=["indices"],
    )


def test_gathernd_example_float32():
    node = onnx.helper.make_node(
        "GatherND",
        inputs=["data", "indices"],
        outputs=["output"],
...
def test_gathernd_example_int16_batch_dim1():
    node = onnx.helper.make_node(
        "GatherND",
        inputs=["data", "indices"],
        outputs=["output"],
    ...
    expected_output = np.array([[2, 3], [4, 5]], dtype=np.int16)
    assert np.array_equal(output, expected_output)
    expect(
        node,
        inputs=[data, indices],
        outputs=[output],
        name="test_gathernd_example_int16_batch_dim1",
        initializer=["indices"],
    )


if __name__ == "__main__":
    test_gathernd_example_int16()
    test_gathernd_example_float32()
    test_gathernd_example_int16_batch_dim1()
